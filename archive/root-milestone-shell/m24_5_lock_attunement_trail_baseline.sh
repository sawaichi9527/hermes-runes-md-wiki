...
p = Path("wiki/k6-freelancer/verification-m24-attunement-trail.md")
s = p.read_text(encoding="utf-8")

# Rewrite verification file to the locked M24.5 baseline.
p.write_text("""# M24 Runes Attunement Trail Verification
...
python3 tools/runes/smoke_m24_4_attunement_trail.py >/tmp/m24_5_smoke.json

python3 - <<'PY'
import json
from pathlib import Path
p = Path("/tmp/m24_5_smoke.json")
text = p.read_text(encoding="utf-8")
start = text.find("{")
data = json.loads(text[start:])
assert data["status"] == "PASS", data
assert data["failed"] == 0, data
assert data["total"] == 73, data
print("M24.5 smoke confirmation: PASS failed=0 total=73")
PY

grep -n "STABLE GOVERNED ATTUNEMENT-HISTORY BASELINE" \
...
git commit -m "Lock M24 attunement trail baseline"
