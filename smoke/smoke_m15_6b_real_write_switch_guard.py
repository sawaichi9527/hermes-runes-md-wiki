...
SMOKE_NOTE = ROOT / "wiki" / "sample-project" / "project-m15-6b-switch-smoke-overview.md"


def run_case(manifest_dir: Path, lock_path: Path, title: str, extra_args: list[str]) -> dict:
    env = os.environ.copy()
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert double_confirm["mode"] == "real-write"
            assert checks["execute_requested"] is True
            ...
            assert double_confirm["plan"]["wiki_write"] is True
            assert double_confirm["plan"]["index_update"] is False
            assert double_confirm["plan"]["note_create"] is True
            assert SMOKE_NOTE.exists()
    finally:
        if SMOKE_NOTE.exists():
            SMOKE_NOTE.unlink()
