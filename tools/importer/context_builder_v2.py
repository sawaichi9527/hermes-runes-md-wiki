...
- M8.1d adds local parent-first grouping:
  - Preserve global retrieval relevance by original hit order.
  - For each original hit, recover same-section neighbors.
  - Locally sort the hit + neighbors by chunk_index before emitting the group.
  - Avoid M8.1c reverse neighbor attachment such as chunk 3 before chunk 2.
  - Avoid M8.1b global path/section reorder regression.
...
def annotate(
    ...
    *,
    ...
    cloned["_m8_local_group_rank"] = local_group_rank
...
def find_neighbors_for_hit(hit: dict[str, Any], args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    - M8.1d uses these neighbors only inside the original hit local group.
    """
...
def build_local_group(hit: dict[str, Any], original_rank: int, args: argparse.Namespace) -> list[dict[str, Any]]:
    """
    Parent-first local group:
    - collect hit + neighbors
    - deduplicate inside the group
    - sort locally by chunk_index
    - preserve group position by original retrieval rank
    """
    group_raw = [hit]
    group_raw.extend(find_neighbors_for_hit(hit, args))

    by_key: dict[tuple[Any, Any, Any], dict[str, Any]] = {}
    hit_key = chunk_key(hit)
    for item in group_raw:
        by_key.setdefault(chunk_key(item), item)

    sorted_group = sorted(by_key.values(), key=chunk_index)

    annotated: list[dict[str, Any]] = []
    for local_rank, item in enumerate(sorted_group, start=1):
        is_hit = chunk_key(item) == hit_key
        annotated.append(
            annotate(
                item,
                original_rank=original_rank,
                local_group_rank=local_rank,
                recovered=not is_hit,
                reason=None if is_hit else "local-parent-first-neighbor",
                parent_chunk_id=None if is_hit else hit.get("chunk_id"),
            )
        )
    return annotated
...
def select_items(recall_data: dict[str, Any], args: argparse.Namespace) -> list[dict[str, Any]]:
    """
    M8.1d selection policy:
    - Preserve raw retrieval order at group level.
    - For each original hit, emit a local parent-first group sorted by chunk_index.
    ...
    for original_rank, hit in enumerate(raw_results, start=1):
        ...
        group = build_local_group(hit, original_rank, args)
...
def build_context(recall_data: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    for idx, item in enumerate(selected, start=1):
        ...
        source = SourceEntry(
            ...
            local_group_rank=item.get("_m8_local_group_rank"),
        ...
        block = (
            ...
            f"Local Group Rank: {source.local_group_rank}\n"
    ...
    return {
        ...
        "ordering": "retrieval-rank-with-local-parent-first-groups",
